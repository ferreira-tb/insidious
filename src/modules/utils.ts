class Utils {
     ...
     */
     static parseGameDate(date: string): number | null {
        const writtenDate = date.trim().toLowerCase();
        if (!writtenDate.includes('às')) return null;

        // splitDate representa apenas as horas, os minutos e os segundos (possivelmente também os milisegundos).
        ...
        // Se foi ontem ou ser for amanhã, faz a mesma coisa, mas remove ou adiciona 24 horas do resultado.
            ...
            const dayAndMonth: string | number[] = (writtenDate.split(' '))[1];
    ...
    static translateResourceName(res: ResourceList): ResourceListPTBR {
        switch (res) {
